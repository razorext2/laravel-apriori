                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        ...
                        <div>
                            ...
                            <a href="{{ url('/apriori/analisa/cetak/' . $kdPengujian) }}" target="_blank"
                    ...
                    <div class="row mb-3 mt-3">
                        <div class="col-xl-3 col-md-6 mb-2">
                            <div class="card bg-light mb-0 border-0 shadow-none">
                                <div class="card-body p-3">
                                    <div class="d-flex align-items-center">
                                        <div class="avatar-xs mr-3">
                                            <span class="avatar-title rounded-circle bg-info font-size-16 text-white">
                                                <i class="mdi mdi-buffer"></i>
                                            </span>
                                        </div>
                                        <div>
                                            <p class="text-muted font-size-12 mb-0">Frequent Itemsets</p>
                                            <h5 class="font-weight-bold text-info mb-0">
                                                {{ $dataPengujian->total_frequent_itemsets ?? count($dataMinSupport) }}
                                                Itemset
                                            </h5>
                                        </div>
                                    </div>
                        ...
                        <div class="col-xl-3 col-md-6 mb-2">
                            <div class="card bg-light mb-0 border-0 shadow-none">
                                <div class="card-body p-3">
                                    <div class="d-flex align-items-center">
                                        <div class="avatar-xs mr-3">
                                            <span class="avatar-title rounded-circle bg-dark font-size-16 text-white">
                                                <i class="mdi mdi-ray-start-arrow"></i>
                                            </span>
                                        </div>
                                        <div>
                                            <p class="text-muted font-size-12 mb-0">Aturan Asosiasi (Rules)</p>
                                            <h5 class="font-weight-bold text-dark mb-0">
                                                {{ $dataPengujian->total_rules ?? count($dataMinConfidence) }} Pola
                                                Terbentuk
                                            </h5>
                                        </div>
                                    </div>
                    ...
                    <div class="table-responsive">
                        <table class="table-hover table-striped mb-0 table" id="tblDataSupport">
                            <thead>
                                <tr>
                                    <th>#</th>
                                    <th>Kode Produk</th>
                            ...
                            <tbody>
                                @foreach ($dataSupport as $supp)
                                    <tr>
                                        ...
                                        <td><strong>{{ $supp->dataProduk($supp->kd_produk)->nama_produk ?? '-' }}</strong>
                                        </td>
                                        <td>{{ $supp->totalTransaksi($supp->kd_produk) }} transaksi</td>
                                        <td>
                                            ({{ $supp->totalTransaksi($supp->kd_produk) }} / {{ $totalTransaksi }})
                                            &times; 100%
                                        </td>
                                        <td><span class="badge badge-primary font-size-13">{{ $supp->support }} %</span>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
                    ...
                    <div class="table-responsive">
                        <table class="table-hover table-bordered mb-0 table" id="tblDataSupportMin">
                            <thead>
                                <tr>
                                    <th>#</th>
                                    <th>Kode Produk</th>
                            ...
                            <tbody>
                                @foreach ($dataMinSupport as $minSupp)
                                    <tr>
                                        ...
                                        <td><strong>{{ $minSupp->dataProduk($minSupp->kd_produk)->nama_produk ?? '-' }}</strong>
                                        </td>
                    ...
                    <div class="table-responsive">
                        <table class="table-hover table-bordered mb-0 table" id="tblKombinasiItemset">
                            <thead>
                                <tr>
                                    <th>#</th>
                            ...
                            <tbody>
                                @foreach ($dataKombinasiItemset as $kombinasi)
                                    <tr>
                                        ...
                                        <td><span
                                                class="badge badge-info font-size-12">{{ $kombinasi->lift_ratio > 0 ? $kombinasi->lift_ratio : '-' }}</span>
                                        </td>
                                        <td>
                    ...
                    <hr />
                    <h5 class="mt-4">4. Pola Aturan Asosiasi Final (&ge; {{ $dataPengujian->min_confidence }}%
                        Confidence)</h5>
                    <div class="table-responsive">
                        <table class="table-hover table-bordered mb-0 table" id="tblPolaHasil">
                            ...
                            <tbody>
                                @forelse($dataMinConfidence as $is)
                                    <tr>
                                        <td>{{ $loop->iteration }}</td>
                                        <td>
                                            Apabila pelanggan memesan layanan
                                            <b>{{ $is->dataProduk($is->kd_barang_a)->nama_produk ?? '-' }}</b>,
                                            maka pelanggan cenderung juga memesan
                                            <b>{{ $is->dataProduk($is->kd_barang_b)->nama_produk ?? '-' }}</b>
                                        </td>
                                        <td>{{ $is->support }} %</td>
                                        <td><span class="badge badge-success font-size-14">{{ $is->confidence }} %</span>
                                        </td>
                                        <td><span
                                                class="badge badge-info font-size-13">{{ $is->lift_ratio > 0 ? $is->lift_ratio : '-' }}</span>
                                        </td>
                                        <td>
                                            @if ($is->lift_ratio > 1.0)
                                                <span class="text-success font-weight-bold"><i
                                                        class="mdi mdi-trending-up mr-1"></i>Korelasi Positif &amp;
                                                    Kuat</span>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="6" class="text-muted text-center">Tidak ada pola yang memenuhi
                                            ambang batas minimum Confidence saat ini. Silakan sesuaikan nilai parameter di
                                            form setup.</td>
                                    </tr>
                                @endforelse
                            </tbody>
