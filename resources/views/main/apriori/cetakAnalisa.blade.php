        ...
        <div class="row mb-3">
            <div class="col-12">
                <table style="border: none; margin-bottom: 20px;">
                    ...
                    <tr style="border: none;">
                        <td style="border: none;"><strong>Waktu Eksekusi API</strong></td>
                ...
                <table class="table-bordered table-striped mt-2 table">
                    <thead>
                        <tr>
                            ...
                            <th style="width: 110px; text-align: center;">Support (A &cup; B)</th>
                            <th style="width: 110px; text-align: center;">Confidence (A &rArr; B)</th>
                            <th style="width: 90px; text-align: center;">Lift Ratio</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse($dataMinConfidence as $is)
                            <tr>
                                ...
                                <td>
                                    Apabila pelanggan memesan
                                    <strong>{{ $is->dataProduk($is->kd_barang_a)->nama_produk ?? '-' }}</strong>,
                                    maka pelanggan cenderung juga memesan
                                    <strong>{{ $is->dataProduk($is->kd_barang_b)->nama_produk ?? '-' }}</strong>
                                </td>
                                <td style="text-align: center;">{{ $is->support }} %</td>
                                <td style="text-align: center;"><strong>{{ $is->confidence }} %</strong></td>
                                <td style="text-align: center;">{{ $is->lift_ratio > 0 ? $is->lift_ratio : '-' }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5" class="text-muted text-center">Tidak ada pola yang memenuhi kriteria
                                    minimum Support & Confidence.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
